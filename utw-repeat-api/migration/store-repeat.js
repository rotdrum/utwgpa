                  ...
                  db.query(
                    ...
                    (err, teac) => {
                      if (teac && teac[0]) {
                        ...
                        dbGrade.query(
                          'insert into `repeat` (student_id, student, status, department_id, subject_id, subject_title, subject_code, subject_class, teacher_id, grade_old, round_at, term_at, year_at, teacher) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?) ', 
                          [std.id, JSON.stringify(std), 'wait_register', row.department_id, row.subject_id, row.subject_title, row.subject_code, row.subject_class, row.user_id, old_grade, settings?.round_at, settings?.term_at, settings?.year_at, teachers]
                        , (err, resp) => {
                          if (err) throw err;
                            if(!err) {
                        ...
                        dbGrade.query(
                          'insert into `repeat` (student_id, student, status, department_id, subject_id, subject_title, subject_code, subject_class, teacher_id, grade_old, round_at, term_at, year_at, teacher) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?) ', 
                          [std.id, JSON.stringify(std), 'wait_register', row.department_id, row.subject_id, row.subject_title, row.subject_code, row.subject_class, row.user_id, old_grade, settings?.round_at, settings?.term_at, settings?.year_at, null]
                        , (err, resp) => {
                          if (err) throw err;
                            if(!err) {
                              console.log("create id course " + row.id + " : success");
            ...
            if (errAdd === false) {
              dbGrade.query('update groub_course set activity = ? WHERE id = ? ', [
                JSON.stringify(stdFail), row.g_id
                  ...
                  if(!err) {
                    console.log("update store-repeat complate ");
                  } else {
                    return error(res, err);
                  }
              })
            } else {
              console.log("update  store-repeat not delete ");
            }
          }
