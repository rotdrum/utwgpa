...
function getOne(req, res) {
  try {
    dbGrade.query("select subject_id, subject_code, subject_title, subject_class from `repeat` WHERE teacher_id = ? GROUP BY subject_id, subject_code, subject_title, subject_class ORDER BY subject_id" , [
      req.params.id
    ], (err, data) => {
      if (data && data[0]) {
