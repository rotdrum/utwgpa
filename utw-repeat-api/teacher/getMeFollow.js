...
function getOne(req, res) {
  try {
    dbGrade.query("select * from `repeat` WHERE teacher_id = ? and (status = ? or status = ? or status = ? or status = ?) ORDER BY subject_id", [
      req.params.id, 'enrol', 'progress', 'wait_success', 'success'
    ], (err, data) => {
      if (data && data[0]) {
