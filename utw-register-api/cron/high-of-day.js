...
function getStudentSuccess(dateSetting) {
  try {
    db.query(
      `select * from register_high where status = ? and is_report = ?`,
      ['approve', 0],
      (err, data) => {
        if (err) {
          console.log(err);
          ...
          if (
            ...
            console.log("cron out time: high-of-day");
          }
        }
      }
    );
...
function addSumCount(amount) {
  db.query('insert into report_register_high_daily (amount) values (?) ', [
    amount
  ], (err, resp, field) => {
      if(!err) {
        const updated_at = new Date();
        db.query('update register_high set is_report = ?, updated_at = ? WHERE status = ? and is_report = ? ', [
