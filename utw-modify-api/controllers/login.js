...
module.exports = async function (req, res) {
  try {
    ...
    var access_token = MD5($_POST['email']) + '@' + MD5($_POST['auth_role']) + '@' + MD5($currentDate);
    var email = req.body.email;
    var updated_at = getDateNow()

    if (txt_email) {
      var [data1] = await dbCors.query(`SELECT * FROM user WHERE email = ? AND access_token = ?`,
        [txt_email, txt_access_token]
      );
      if (data1 && data1[0]) {
       await dbCors.query(`UPDATE user SET access_token = ?, updated_at = ? WHERE id = ?`,
        ...
        return success(res, [{
          ...
          "lname": data1[0].lname,
          "email": data1[0].email,
          "auth_role": data1[0].auth_role,
          "department_id": data1[0].department_id,
