...
<html>
 <head>
    ...
    <title>Login ระบบลงทะเบียนเรียนซ้ำ</title>
    <meta charset="utf-8">
...
.box1{
    width: 30px;
    height: 500px ;
    background: linear-gradient(to left,#2ecc71, #16a085, #16a085);
    background-size: 400%;
...
.indexBG {
  z-index: 1; position: relative; width: 100%; height: 100vh; background-size: 400% !important; animation: slidetothejail 10s ease infinite; display: flex; flex-direction: column; justify-content: center; align-items: center; background: linear-gradient(-45deg, #16a085, #2ecc71, #16a085);
}
    </style>
    <?php
...
if(strpos($email,"@utw") > 0 ) {
  ...
  curl_setopt_array($curl, array(
      CURLOPT_URL => $endpointNew.'/utw-cors-api/check-mail.php',
      ...
      CURLOPT_POSTFIELDS => '_method=GET&email='.$email,
  ...
  // print_r($obj);
  if($obj->status->code == 201) { //student
  ...
  else if($obj->status->code == 200) { //teacher
...
?>
 ...
 <body style="background: #F9F9F9; width: 100%; height: 100vh; display: flex; flex-direction: column; align-items: center; justify-content: center;">
  ...
  <script>
    ...
    const alertx = urlx.searchParams.get("alert");
    if(alertx) {
      Swal.fire({
        icon: "error",
        title: alertx,
